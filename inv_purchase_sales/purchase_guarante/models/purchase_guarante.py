...
class PurchaseGuarante(models.Model):
    ...
    name = fields.Char(string="Guarantee Name", compute="_compute_name", store=True)
    ...
    customs_guaranteed_amount = fields.Monetary(string="Customs Guaranteed Amount", required=True)
    guaranteed_amount_before_interest = fields.Monetary(string="Guaranteed Amount (75%)", compute="_compute_guaranteed_amount", store=True, readonly=True)
    paid_amount = fields.Monetary(string="Paid Amount (25%)", compute="_compute_guaranteed_amount", store=True, readonly=True)
    interest_amount = fields.Monetary(string="Interest Amount", compute="_compute_interest_amount", store=True, readonly=True)
    guaranteed_amount_after_interest = fields.Monetary(string="Guaranteed Amount After Interest", compute="_compute_interest_amount", store=True, readonly=True)
    lc_number = fields.Char(related='purchase_order_id.lc_number', string="LC Number")
    lc_amount = fields.Float(related='purchase_order_id.lc_amount', string="LC Amount")
    declaration_number = fields.Char(string="Declaration Number", required=True)
    ...
    customs_branch_id = fields.Many2one('customs.branch', string="Customs Branch")
    account_type = fields.Selection(related='customs_branch_id.account_type', string="Account Type", store=True)
    account_number = fields.Char(related='customs_branch_id.account_number', string="Account Number", store=True)
    ...
    def _check_customs_guaranteed_amount(self):
        for record in self:
            if record.customs_guaranteed_amount <= 0:
                raise ValidationError("Customs guaranteed amount must be positive.")
    ...
    def _compute_payment_lines(self):
        for guarantee in self:
            ...
            if guarantee.guarantee_period > 0 and guarantee.start_date and guarantee.guaranteed_amount_before_interest > 0 and guarantee.customs_branch_id.monthly_payment:
                monthly_amount = guarantee.guaranteed_amount_before_interest / guarantee.guarantee_period
                
                lines_to_create = []
                for month_num in range(1, guarantee.guarantee_period + 1):
                    due_date = guarantee.start_date + relativedelta(months=month_num)
    ...
    def _compute_expiry_date(self):
        for record in self:
            if record.start_date and record.guarantee_period:
                record.expiry_date = record.start_date + relativedelta(months=record.guarantee_period)
            else:
    ...
    def _check_guarantee_period(self):
        for record in self:
            if record.guarantee_period <= 0 or record.guarantee_period > 12:
                raise ValidationError("Guarantee period must be positive and less than or equal to 12.")
    ...
    def _compute_interest_amount(self):
        for record in self:
            ...
            record.interest_amount = record.guaranteed_amount_before_interest * 0.215 * month * 30 / 365
            record.guaranteed_amount_after_interest = record.guaranteed_amount_before_interest + record.interest_amount
    ...
    def _cron_guarantee_reminders(self):
        ...
        _logger.info("Starting cron job for final guarantee expiry reminders...")
        today = date.today()

        records_to_expire = self.search([
            ('expiry_date', '<', today),
            ('state', 'in', ['in_progress', 'extended']),
            ('customs_branch_id.monthly_payment', '=', False) # <-- Only for non-monthly
        ])
        ...
        activity_type = self.env.ref("purchase_guarante.mail_act_purchase_guarantee_to_renew", raise_if_not_found=False)
        if not activity_type:
            _logger.warning("Activity type 'Guarantee to Renew' not found. Skipping final expiry cron.")
            return

        notification_group = self.env.ref('purchase_guarante.purchase_guarantee_group_expiry', raise_if_not_found=False)
        if not notification_group or not notification_group.users:
            _logger.warning("Group 'purchase_guarantee_group_expiry' has no users. Skipping final expiry cron.")
            return
        ...
        for days in reminders:
            ...
            expiring_guarantees = self.search([
                ...
                ('customs_branch_id.monthly_payment', '=', False) # <-- ADDED THIS CRITICAL CONDITION
            ])
            
            _logger.info(f"Found {len(expiring_guarantees)} non-monthly guarantees expiring in {days} days.")
    ...
    def _cron_payment_deadline_reminders(self):
        ...
        try:
            activity_type = self.env.ref("purchase_guarante.mail_act_payment_deadline_reminder")
            notification_group = self.env.ref('purchase_guarante.purchase_guarantee_group_payment_reminders')
        except ValueError:
            _logger.warning("Required data (activity type or user group) for payment reminders not found. Skipping.")
            return

        if not notification_group or not notification_group.users:
            _logger.warning("Group 'purchase_guarantee_group_payment_reminders' has no users. Skipping reminder cron.")
            return
        ...
        monthly_lines = PaymentLine.search([
            ...
            ('guarantee_id.customs_branch_id.monthly_payment', '=', True), # <-- MODIFIED CONDITION
        ])

        _logger.info(f"Found {len(monthly_lines)} payment lines for monthly payment branches due on {target_date}.")
        for line in monthly_lines:
            summary = _(f"Payment due in 7 days for {line.guarantee_id.name}")
            note = _(f"Monthly payment of {line.total_amount} {line.currency_id.symbol} for guarantee {line.guarantee_id.name} is due on {line.payment_date}.")
            line._create_reminder_activity(activity_type, users_to_notify, summary, note)
...
class PurchaseGuarantePaymentLine(models.Model):
    ...
    guarantee_id = fields.Many2one('purchase.guarante', string="Guarantee", required=True, ondelete='cascade')
    company_id = fields.Many2one(related='guarantee_id.company_id', store=True)
    currency_id = fields.Many2one(related='guarantee_id.currency_id', store=True)
    ...
    def _create_reminder_activity(self, activity_type, users, summary, note):
        ...
        self.ensure_one()
        model_id = self.env.ref('purchase_guarante.model_purchase_guarante_payment_line').id
        for user in users:
            if not self.env["mail.activity"].search_count([
                ...
                _logger.info(f"Created payment reminder for user {user.name} on payment line {self.name}.")
...
class CustomsBranch(models.Model):
    ...
    guarantee_ids = fields.One2many('purchase.guarante', 'customs_branch_id', string="Guarantees")
