...
class LunchLog(models.Model):
    ...
    name = fields.Char(default=lambda self: self.env['ir.sequence'].next_by_code(
        'lunch.log.sequence'), required=True, readonly=True)
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('ordered', 'Ordered'), ('unbilled', 'Unbilled'), ('billed', 'Billed')],
        string='Status',
        default='draft',
        readonly=True,
        copy=False,
    ...
    def _compute_is_current_user_employee(self):
        ...
        for log in self:
            if log.employee_id and current_user_employee:
                log.is_current_user_employee = (log.employee_id == current_user_employee)
            else:
