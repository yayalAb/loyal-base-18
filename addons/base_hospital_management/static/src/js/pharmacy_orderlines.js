...
export class PharmacyOrderLines extends Component {
    ...
    async create_order() {
        ...
        // Update the symbols in the DOM using vanilla JS
        const symbolCurrencyEl = document.getElementById(`symbol${currency}`)
        if (symbolCurrencyEl) {
            symbolCurrencyEl.textContent = result || ""
        }
        const symbolEl = document.getElementById("symbol")
        if (symbolEl) {
            symbolEl.textContent = result || ""
        }

        this.state.medicines = this.product_lst
        this.state.units = this.uom_lst
    }
    ...
    async _onChange_prod_price(med_id) {
        ...
        const medicine = this.state.medicines.filter(
            med => med.id === med_id
        )[0]
        this.lineState.price = medicine.list_price
        this.lineState.sub_total = this.calculateSubtotal(
            this.lineState.qty,
            medicine.list_price
        )
