...
export class LabDashBoard extends Component {
    ...
    async _loadTestData() {
        ...
        // Track the current model if needed
        this.state.modelName = "lab.test.line"

        const domain = [["state", "=", "draft"]]
        const result = await this.orm.call("lab.test.line", "search_read", [
            domain,
        ])
        this.state.tests_confirm = result

        // Hide the form view element (using vanilla JS)
        const formViewEl = document.getElementById("form-view")
        if (formViewEl) {
            formViewEl.hidden = true
        }

        // Update the create button content
        const createButtonEl = document.getElementById("create-button")
        if (createButtonEl) {
            createButtonEl.innerHTML =
                '<button class="btn btn-outline-info" id="create" style="margin-left:10px;">Create</button>'
        }
    }
    ...
    confirmLabTest() {
        this.orm
            .call("lab.test.line", "create_lab_tests", [this.record_id])
