...
class ResCompany(models.Model):
    ...
    external_ref_sequence_id = fields.Many2one('ir.sequence', string='External Reference Sequence')
    ...
    fiscal_year_for_sequence = fields.Char(
        ...
        default=lambda self: str(date.today().year - 8), # Sensible default for Ethiopian year
    ...
    def _get_formatted_sequence(self, doc_type):
        ...
        if not self.fiscal_year_for_sequence:
            raise UserError(_("The 'Current Fiscal Year for Sequences' is not set in General Settings. Please configure it."))
        ...
        if doc_type == 'external':
            ...
            error_msg = _("The sequence for External References is not configured in General Settings.")
            ...
            error_msg = _("The sequence for Internal References is not configured in General Settings.")
        else:
            return ''
